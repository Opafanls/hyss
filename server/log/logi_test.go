...
func TestLogrusWithCtx(t *testing.T) {
	...
	logger.Tracef(GetCtxWithLogID(ctx, "tag"), "Tracef: name:%s, err: %+v, data: %+v", name, err, data)
	logger.Debugf(GetCtxWithLogID(ctx, "tag"), "Debugf: name:%s, err: %+v, data: %+v", name, err, data)
	logger.Infof(GetCtxWithLogID(ctx, "tag"), "Infof: name:%s, err: %+v, data: %+v", name, err, data)
	logger.Warnf(GetCtxWithLogID(ctx, "tag"), "Warnf: name:%s, err: %+v, data: %+v", name, err, data)
	logger.Errorf(GetCtxWithLogID(ctx, "tag"), "Errorf: name:%s, err: %+v, data: %+v", name, err, data)
}
