...
import (
	...
	"crypto/sha256"
	"fmt"
	pio2 "github.com/Opafanls/hylan/server/util/pio"
	"io"
...
func hsCreate01(p []byte, time uint32, ver uint32, key []byte) {
	...
	p1 := p[1:]
	rand.Read(p1[8:])
	pio2.PutU32BE(p1[0:4], time)
	pio2.PutU32BE(p1[4:8], ver)
...
func (conn *Conn) HandshakeClient() (err error) {
	...
	S1 := S0S1S2[1 : 1536+1]
	if ver := pio2.U32BE(S1[4:8]); ver != 0 {
		C2 = S1
	} else {
		C2 = S1
	}
...
func (conn *Conn) HandshakeServer() (err error) {
	...
	clitime := pio2.U32BE(C1[0:4])
	srvtime := clitime
	srvver := uint32(0x0d0e0a0d)
	cliver := pio2.U32BE(C1[4:8])
